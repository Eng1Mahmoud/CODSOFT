...
const TestForm = () => {
    ...
    const addQuestion = () => {
        ...
        if (currentQuestion.question === "" || trimmedOptions.some(option => option === "")) {
        ...
        if (! trimmedOptions.includes(trimmedCorrectAnswer)) {
            setAddedTest({added: true, message: "Please ensure the correct answer exists in the options.", error: true});
            return;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post("https://quizs-wg02.onrender.com/api/addTest", {
                testName,
                questions
            }, {
                headers: {
        ...
        <div className="container-test">
            ...
            addedTest.added ? (
                <div className="popup"
                    style={
                        {
                            background: addedTest.error ? "red" : "green"
                        }
                }>
                    <p>{
                        addedTest.message
